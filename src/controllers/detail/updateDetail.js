...
const updateDetail = async (req, res) => {
    try {
        const { codigo_servicio, cod_servicio, nro_actividad } = req.params;
        const { horas_requeridas } = req.body;
        const verify = await pool.query('SELECT * FROM orden_de_servicio WHERE codigo = $1', [codigo_servicio]);
        ...
        const verify3 = await pool.query('SELECT * FROM DETALLA WHERE codigo_orden_servicio = $1 AND codigo_servicio = $2 AND nro_actividad = $3', [codigo_servicio, cod_servicio, nro_actividad]);
        ...
        if (verify.rows.length === 0) {
            ...
            if (verify2.rows.length === 0 || verify4.rows.length === 0) {
                ...
                if (verify3.rows.length !== 0) {
                    ...
                    const response = await pool.query('UPDATE DETALLA SET horas_requeridas = $1 WHERE codigo_orden_servicio = $2 AND codigo_servicio = $3 AND nro_actividad = $4 RETURNING *', [horas_requeridas, codigo_servicio, cod_servicio, nro_actividad]);
                    res.status(200).json({
                        success: true,
