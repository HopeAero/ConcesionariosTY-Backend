...
CREATE TABLE MANTENIMIENTO_RECOMENDADO (
    placa_vehiculo VARCHAR(10) NOT NULL,
    codigo_modelo INTEGER NOT NULL,
    codigo_servicio INTEGER NOT NULL,
    CONSTRAINT pk_mantenimiento_recomendado PRIMARY KEY (placa_vehiculo, codigo_modelo, codigo_servicio),
    CONSTRAINT fk_M_placa_vehiculo FOREIGN KEY (placa_vehiculo) REFERENCES VEHICULO(placa) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_M_modelo FOREIGN KEY (codigo_modelo) REFERENCES MODELO(codigo) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_M_servicio FOREIGN KEY (codigo_servicio) REFERENCES SERVICIO(codigo) ON UPDATE CASCADE ON DELETE RESTRICT
);